...
description: Java 8 및 Spring Boot 2.6.11 기반의 백엔드 로직 구현 및 아키텍처 전문가. 서비스 계층 구현, 트랜잭션 처리, Controller/Security 설정 작업 시 사용.
---
...
### CommonUtils.getIP() 동작 주의
`CommonUtils.getIP(request)`는 요청 IP가 `127.0.0.1` 또는 `0:0:0:0:0:0:0:1`(루프백)이면 `InetAddress.getLocalHost().getHostAddress()`로 교체합니다. 로컬 개발 환경에서 VirtualBox 등 가상 네트워크 어댑터가 있으면 실제 호스트 IP가 달라질 수 있습니다. 감사 필드(`insIp`, `updIp`)에 의도한 IP가 기록되지 않는 경우 이 동작을 먼저 확인합니다.
...
- **필수 테스트 프로퍼티**: `application-test.properties` 작성 시 다음 항목이 누락되면 `ApplicationContext` 로딩이 실패함:
    ...
    - COS 속성 6개(`cos.endpoint`, `cos.location`, `cos.api-key`, `cos.service-instance-id`, `cos.iam_serviceid_crn`, `cos.bucket`): `@MockBean`으로 빈을 대체해도 `S3Config` 클래스 자체의 `@Value` 주입은 발생하므로 fallback 기본값 필요.
- **환경 변수 fallback 패턴**: `${VAR_NAME:default_value}` 형식으로 작성하면 CI는 환경 변수를 사용하고 로컬은 기본값으로 동작함.
