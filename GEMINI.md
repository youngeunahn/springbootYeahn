...
- **MyBatis 표준:** 기존 `@Repository` 클래스 방식 대신 **`@Mapper` 인터페이스 방식**을 지향하며, XML 네임스페이스를 인터페이스의 전체 경로로 설정함.
...
## 개발 프로세스 및 검증
- **서브에이전트 검토:** 주요 로직 구현 전후로 분야별 전문 서브에이전트(`springboot-expert`, `mybatis-expert` 등)에게 검토를 요청하여 코드 품질과 기술적 일관성을 보장함.
- **단위 테스트:** 로직 변경 시 반드시 관련 단위 테스트(JUnit 5)를 수행하고 성공 여부를 확인함.
